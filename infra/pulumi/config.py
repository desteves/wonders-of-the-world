...
# Optional Google Cloud configuration for Cloud Function deployment
GCP_PROJECT = gcp_config.get("project") or os.getenv("GCP_PROJECT")
GCP_REGION = gcp_config.get("region") or os.getenv("GCP_REGION")
GCP_CREDENTIALS = gcp_config.get_secret("credentials") or os.getenv("GOOGLE_CREDENTIALS") or os.getenv("GOOGLE_APPLICATION_CREDENTIALS")

if not GCP_PROJECT:
    print(
        "ℹ️  Missing GCP project ID. Provide one with\n"
        "pulumi config set gcp:project <GCP_PROJECT_ID>\n"
        "if you intend to deploy the Cloud Function."
    )

if not GCP_REGION:
    print(
        "ℹ️  Missing GCP region. Provide one with\n"
        "pulumi config set gcp:region <GCP_REGION>\n"
        "when preparing the Cloud Function deployment."
    )

if not GCP_CREDENTIALS:
    print(
        "ℹ️  GCP credentials not detected. Supply a service account JSON with\n"
        "pulumi config set --secret gcp:credentials @path/to/key.json\n"
        "or export GOOGLE_APPLICATION_CREDENTIALS before deploying to Cloud Functions."
    )

GCP_FUNCTION_IP = pulumi_config.get("gcp_function_ip") or os.getenv("GCP_FUNCTION_IP")
